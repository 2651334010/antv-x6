  ...
  <div style="width: 100%;height: 100%;">
    <el-tabs>
      <el-tab-pane label="线条">
        ...
        <el-row>
          <el-col :span="8">Enable Animation</el-col>
          <el-col :span="14">
            <el-switch
              v-model="isAnimationEnabled"
              @change="onAnimationEnabledChange"
            />
          </el-col>
        </el-row>
		<el-row v-show="isAnimationEnabled">
		  <el-col :span="8">Start Animation</el-col>
		  <el-col :span="14">
			<el-switch v-model="startAnimation" @change="onStartAnimationChange"/>
		  </el-col>
		</el-row>
		 <!-- Animation Color Picker -->
        <el-row v-show="isAnimationEnabled">
          <el-col :span="8">Animation Color</el-col>
          <el-col :span="14">
            <el-color-picker
              v-model="animationColor"
              @change="onAnimationColorChange"
            />
          </el-col>
        </el-row>
      </el-tab-pane>
...
export default {
  ...
  watch: {
    id: {
      handler() {
		...
		this.edgeRouter = cell.getRouter()?.name || "normal";
      if (this.edgeRouter === 'er') {
        const args = cell.getRouter()?.args || {};
        this.edgeOffset = args.offset || 8;
        this.edgeDirection = args.direction || 'L';
      }
			// Read data attributes
		  const data = cell.getData();
		  if(data!=null){
			  this.isAnimationEnabled = data.isAnimationEnabled || false;
			  this.animationColor = data.animationColor || '#000000';
			  this.startAnimation = data.startAnimation || false;
		  }else{
			  this.isAnimationEnabled = false;
			  this.animationColor = '#000000';
	...
	edgeRouter: {
		handler() {
		  this.onEdgeRouter(); // 确保在 edgeRouter 变化时立即更新路由设置
		},
		immediate: true // 如果需要即时更新，可以设置为 true
	  },
    edgeOffset: {
      handler() {
  ...
  methods: {
	...
	onAnimationEnabledChange() {
      this.curCell?.attr("line/style/animation", this.isAnimationEnabled);
      this.curCell?.setData({isAnimationEnabled:this.isAnimationEnabled});
    },
	onStartAnimationChange() {
      this.curCell?.attr("line/style/startAnimation", this.startAnimation);
	  this.curCell?.setData({startAnimation:this.startAnimation});
    },
    onAnimationColorChange() {
      const animationStyle = this.isAnimationEnabled ? this.animationColor : 'none';
      this.curCell?.attr("line/style/animationColor", animationStyle);
	  this.curCell?.setData({animationColor:this.animationColor});
    },
    // 连接方式
    onConnectorChange () {
    ...
    // 路由
    onEdgeRouter() {
	  console.log("Setting router:", this.edgeRouter, this.edgeOffset, this.edgeDirection);
	  if (this.curCell) {
		this.curCell.setRouter({
		  name: this.edgeRouter,
		  args: {
